...
class WeightedEdge(Edge):
    def __init__(self, src: Node, dest: Node, total_distance: int, outdoor_distance: int):
        self.src = src
        self.dest = dest
...
class Digraph(object):
    ...
    def get_edges_for_node(self, node: Node):
    ...
    def add_node(self, node: Node):
        ...
        if node in self.nodes:
            raise ValueError(f"AddNodeError: Node={node} already in {self.nodes}")
        node_hash = node.__hash__()

        if node_hash in self.edges:
            raise ValueError(f"AddNodeError: Node={node} already in {self.nodes}")
        self.nodes.add(node)
        self.edges[node_hash] = []

    def add_edge(self, edge: (WeightedEdge | Edge)):
        """Adds an Edge or WeightedEdge instance to the Digraph. Raises a
        ValueError if either of the nodes associated with the edge is not
        in the  graph."""
        if edge.src not in self.nodes:
            raise ValueError(f"AddEdgeError: Edge has no source Node.")
        if edge.dest not in self.nodes:
            raise ValueError(f"AddEdgeError: Edge has no destination Node.")
        src_node_hash = edge.src.__hash__()

        if src_node_hash not in self.edges:
            raise ValueError(f"AddEdgeError: EdgeHash not found in Edges dict.")
        self.edges[src_node_hash].append(edge)
