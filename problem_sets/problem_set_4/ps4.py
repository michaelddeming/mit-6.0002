...
class SimpleBacteria(object):
    ...
    def reproduce(self, pop_density):
        ...
        overall_prob = self.birth_prob * (1 - pop_density)

        if rand_num <= overall_prob:
            # return a new child obj with same spec
...
class Patient(object):
    ...
    def __init__(self, bacteria, max_pop):
        ...
        self.bacteria = bacteria if bacteria else []
    ...
    def update(self):
        ...
        new_pop_den = len(survivors) / self.max_pop

        offspring = []
        for bacteria in survivors:
            try:
                child = bacteria.reproduce(new_pop_den)
...
def calc_pop_avg(populations, n):
    ...
    """
    return sum([trial[n] for trial in populations]) / len(populations)
...
def simulation_without_antibiotic(num_bacteria,
    ...
    for trial in range(num_trials):
        ...
        bacteria = [SimpleBacteria(birth_prob=birth_prob, death_prob=death_prob) for _ in range(num_bacteria)]

        new_patient = Patient(bacteria=bacteria, max_pop=max_pop)

        runs = [new_patient.get_total_pop()]
        for i in range(300):
            runs.append(new_patient.update())
        res.append(runs)
    
    y_vals = []
    for i in range(301):
        y_vals.append(calc_pop_avg(res, i))


    
    make_one_curve_plot(x_coords=list(range(301)), y_coords=y_vals, x_label="Timestep", y_label="Average Population", title="Without Antibiotic")
    return res
...
def calc_pop_std(populations, t):
    ...
    # populations is a 2D matrix where each list "row" is a trial and each index in said row, "column" is a time step. The value at the timestep index is the number of bacteria.
    n = len(populations)
    average = calc_pop_avg(populations, t)

    square_distance_sum = 0
    for row in populations:
        square_distance_sum += (row[t] - average)**2
    
    std_dev = math.sqrt((1/n)*(square_distance_sum))
    return std_dev
 

# print(calc_pop_std(population, 299))
...
def calc_95_ci(populations, t):
    ...
    return (average, sem * 1.96)

# print(calc_95_ci(population, 299))
...
class ResistantBacteria(SimpleBacteria):
    ...
    def is_killed(self):
        ...
        res = False
        if self.resistant:
            if chance <= self.death_prob:
                res = True
        else:
            if chance <= (self.death_prob / 4):
                res = True
        return res
    ...
    def reproduce(self, pop_density):
        ...
        """
        repo_chance = random.random()
        repo_prob = self.birth_prob * (1 - pop_density)

        if repo_chance <= repo_prob:
            child_resistance = False
            if self.resistant:
                child_resistance = True
            else:
                resistance_chance = random.random()
                resistance_prob = self.mut_prob * (1-pop_density)
                if resistance_chance <= resistance_prob:
                    child_resistance = True
            
            child = ResistantBacteria(birth_prob=self.birth_prob, death_prob=self.death_prob, resistant=child_resistance, mut_prob=self.mut_prob)
            return child
        raise NoChildException()
...
class TreatedPatient(Patient):
    ...
    def get_resist_pop(self):
        ...
        """
        return len([bacteria for bacteria in self.bacteria if bacteria.resistant])
    ...
    def update(self):
        ...
        for bacteria in self.bacteria:
            if not bacteria.is_killed():
                if not self.on_antibiotic or bacteria.resistant:
                    survivors.append(bacteria)
        offspring = []
...
def simulation_with_antibiotic(num_bacteria,
    ...
    for trial in range(num_trials):
        ...
        bacteria = [ResistantBacteria(birth_prob=birth_prob,death_prob=death_prob,resistant=resistant,mut_prob=mut_prob) for _ in range(num_bacteria)]
        ...
        for _ in range(150):
            ...
            patient.update()
            trial_bacteria_counts.append(patient.get_total_pop())
            trial_resistant_counts.append(patient.get_resist_pop())
        ...
        for _ in range(250):
            ...
            trial_bacteria_counts.append(patient.get_total_pop())
            trial_resistant_counts.append(patient.get_resist_pop())
        populations.append(trial_bacteria_counts)
        resistant_pop.append(trial_resistant_counts)
    
    y_coords1 = [calc_pop_avg(populations, i) for i in range(400)]
    y_coords2 = [calc_pop_avg(resistant_pop, i) for i in range(400)]


    make_two_curve_plot(x_coords=list(range(400)), y_coords1=y_coords1, y_coords2=y_coords2, y_name1="Total", y_name2="Resistant",title="With an Antibiotic", x_label="Timestep", y_label="Average Population")
